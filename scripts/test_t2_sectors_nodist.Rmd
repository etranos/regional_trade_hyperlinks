...
models <- lapply(sectors, function(x) paste0(x,"  ~ hl + emp_thousands.orig + emp_thousands.dest + pop_density.orig + pop_density.dest")) #dist + 
...
rf.year.all.metrics %>%
  ...
  mutate(variable = gsub('X', '', variable),
         sector = str_replace(sector, "ss", "s"),
         sector = factor(sector, levels = sector.order)) #%>% write_csv("rmse_nodist.csv")
  ggplot() + 
  aes(x = as.numeric(variable), y = value) +
  geom_line(aes(color = sector), size = 1) +
  labs(x = "year", y = "RMSE") +
