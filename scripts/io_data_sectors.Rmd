...
for(i in files){
  for(j in sectors){
  file <- paste(path, "/data/IO/", i, sep = "")
  #file <- paste("C:/Users/nw19521/DataShare/Regional positioning in GVC and networks (Andre Carrascal Incera)/data/IO/", i, sep = "")
  #file <- "C:/Users/nw19521/DataShare/Regional positioning in GVC and networks (Andre Carrascal Incera)/data/IO/2000.xlsx" 
  y.names <- read_excel(file, col_names = F)
  
  # keep ss2 rows
  y.names <- y.names[y.names$...2 == j | is.na((y.names$...2)),]  
  
  # keep ss2 columns
  y.names <- as.matrix(y.names)
  y.names <- y.names[, y.names[2, ] == j] 
  
  # create a vector with NUTS2 names
  y.names.c <- y.names[(1),-(1:2)]
  #y.names.c <- t(y.names.c)
  y.names.c <- as.data.frame(y.names.c)
  
  # column names
  y.names <- y.names[-2,-(1:2)]
  colnames(y.names) <- y.names[1,]
  y.names <- y.names[-1,]
  y.names <- as.data.frame(y.names)
  
  # convert flows to numeric
  y.names <- sapply(y.names, function(x) as.numeric(as.character(x)))
  
  # from OD to edge list
  y.names <- as.matrix(y.names)
  
  # row.names
  row.names(y.names) <- y.names.c$y.names.c
  
  m <- odmatrix_to_od(y.names)
  ...
  n <- n +1
  #names(m)[names(m) == "flow"] <- paste("y", n, j, sep="") 
  names(m)[names(m) == "flow"] <- paste(i, j, sep="") 
...
total.y <- total.y %>%
  relocate(id, .before=orig)

# removes .xlsx form column names
names(total.y) <- sub(".xlsx", "", names(total.y))
...
### melt and merge sectors with hl

```{r}
# # single sector test
# total.ss1.melt <- reshape2::melt(total.y, measure.vars = grep("ss1", names(total.y), value = T))
# total.ss1.melt <- total.ss1.melt %>%
#   dplyr::select(id, variable, value) %>%
#   filter(grepl("ss1$", variable)) %>%
#   mutate(variable = gsub("ss1", "", variable)) %>%
#   rename(year = variable,
#          ss1 = value)

# loop on sectors
for (s in sectors){
  total.sector.melt <- reshape2::melt(total.y, measure.vars = grep(s, names(total.y), value = T))
  sector <- paste0(s, "$") # for the grepl()
  total.sector.melt <- total.sector.melt %>%
    dplyr::select(id, variable, value) %>%
    dplyr::filter(grepl(sector, variable)) %>%
    mutate(variable = gsub(s, "", variable)) %>%
    rename(year = variable) #, sector.name = value)
  colnames(total.sector.melt)[3] <- paste0(s)
  assign(paste0(s, "melt.sector"), total.sector.melt)
}


sectors <- grep("melt.sector",names(.GlobalEnv),value=TRUE)
sectors <- do.call("list",mget(sectors))

sapply(sectors, function(x) dim(x))
sapply(sectors, function(x) names(x))
...
total.hl.melt <- reshape2::melt(total.y, measure.vars = grep("x", names(total.y), value = T))
total.hl.melt <- total.hl.melt %>%
  dplyr::select(id, orig, dest, variable, value) %>%
  #filter(grepl("x$", variable)) %>%
  mutate(variable = gsub("x", "", variable)) %>%
  rename(year = variable,
         hl = value)

total <- merge(total.hl.melt, sectors, by = c("id", "year"))
...
**TODO**: check if it is spherical distance 
