...
models <- lapply(sectors, function(x) paste0(x,"  ~ hl + dist + emp_thousands.orig + emp_thousands.dest + pop_density.orig + pop_density.dest"))
...
fit.model.all <- mget(ls(pattern = "~ hl"))
...
years <- (2001:2009)
pred.by.year.all <- data.frame(matrix(NA,nrow=172494, ncol = 0)) #1369
for (t in years){
  for (s in sectors){
  t_plus_1 <- t + 1
...
out.path <- paste0(path, "/paper/figures/sector_rsquared.png")
...
stopCluster(cl)
out.path <- paste(path, "/data_inter/test_t2_sectors.RData", sep = "")
