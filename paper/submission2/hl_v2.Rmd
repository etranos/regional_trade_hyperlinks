...
regression trees, can illustrate the predictor importance, and are fast and easy to 
...
predictions of data, which are skewed and have outliers (see for Section [5](#sec:5)).
...
For instance, @pourebrahim2019trip coupled a traditional methodological framework -- gravity and 
...
RF allow the estimation of predictor importance. Using the built-in bootstrapping 
...
of these data are explained in the next section.
...
```{r echo=FALSE, message=FALSE, fig.cap="\\label{Flow_maps} Interregional trade and hyperlink flows", fig.height=15, fig.width=10}
...
fit.model.all.res$values %>% #extract the values
  ...
  ggplot()+ #call ggplot
  geom_boxplot(aes(x = RMSE, y = model)) +
  #theme(axis.text.x=element_text(size=rel(0.5), angle=90)) 
  scale_x_continuous(breaks=seq(0, 6000, 2000)) -> RMSE #and plot the box plot
...
# path <- find_rstudio_root_file()
# path.out <- paste0(path, "/paper/submission2/figures/accuracy.tiff")
# ggsave(path.out, device='tiff', dpi=700, plot=grid.arrange(
#   Rsquared, MAE, RMSE,
#   nrow = 1, ncol = 3))
```
...
```{r echo=FALSE, results= 'markup', message=FALSE, fig.cap="\\label{prediction}Predicted vs. observed interregional trade (in £100,000s) by year"}
...
path <- find_rstudio_root_file()
path.out <- paste0(path, "/paper/submission2/figures/pred_obs.tiff")
ggsave(path.out, device='tiff', dpi=700)
...
difficult to predict. As expected, our model performs worse in some specific sectors. 
The most obvious example is hospitality (s11), for which the R-squared between the 
predicted and observed values for $2004$ and $2010$ drops below $0.25$
and it does not exceed $0.75$ for the whole study period. This can be attributed
...
well in predicting unseen interregional trade flows regarding manufacturing and 
...
path <- find_rstudio_root_file()
path.out <- paste0(path, "/paper/submission2/figures/accuracy_alt.tiff")
ggsave(path.out, device='tiff', dpi=700)
...
```{r echo=FALSE, results= 'markup', message=FALSE, fig.cap="\\label{prediction_multi_pc}Predicted vs. observed interregional trade (in £100,000s) by year for websites with multiple postcodes"}
...
path <- find_rstudio_root_file()
path.out <- paste0(path, "/paper/submission2/figures/pred_obs_alt.tiff")
ggsave(path.out, device='tiff', dpi=700)
...
of our research framework in mapping interregional trade flows at a dissagregated 
...
^[We opted towards presenting the predictions for $2010$ as this was the latest 
year we had data for.].
...
^[The Appendix provides the URL to interactive visualisations of the hypelink
...
```{r, echo=FALSE, message=FALSE, fig.cap="\\label{LAD_Maps}Local Authority Districts trade flow predicitions (in  £100,000s): top left- Birmingham Exports, top right - Birmingham imports, bottom left - Camden exports, bottom right - Camden imports", fig.height=15, fig.width=10}
#test
knitr::include_graphics("figures/Predictions2_updated.png")
```
...
The authors would like to thank the editor and the anonynous reviewers for their
constructive comments. The first author would also like to thank the QuSS Research
Group at the School of Geographical Sciences at the University of Bristol and also
the participants and the organisers of the Spatial Analytics + Data seminar series.

# Author Biographies {-}

EMMANOUIL TRANOS is a Professor in Quantitative Human Geography at the University of
Bristol in the UK and a fellow at the Alan Turing Institute. His research has been 
exposing the spatial dimensions of digital technologies and the digital economy 
from their early stages until today. E-mail: [e.tranos@bristol.ac.uk](mailto:e.tranos@bristol.ac.uk), Twitter: [\@EmmanouilTranos](https://twitter.com/EmmanouilTranos), Personal website: [etranos.info](https://etranos.info/).

\bigskip
\noindent
ANDRÉ CARRASCAL-INCERA is an Assistant Professor in the Department of Economics 
at the University of Oviedo, Asturias, Spain.  
His lines of research are related to the generation and distribution of income in multiregional systems and the development of economy-wide models at the regional level.
E-mail: [carrascalandre@uniovi.es](mailto:carrascalandre@uniovi.es), 
Twitter: [\@Andre_Carrascal](https://twitter.com/Andre_Carrascal),
Personal website: [https://sites.google.com/view/andrecarrascalincera](https://sites.google.com/view/andrecarrascalincera).

\bigskip 
\noindent
GEORGE WILLIS is a doctoral student in the School of Geography at the University 
of Birmingham, UK. His research interests include quantitative methods in Geography, 
focusing on the spatiotemporal patterns of urban shrinkage in China.
E-mail: [gcw519@bham.ac.uk](mailto:gcw519@bham.ac.uk),
Twitter: [\@GeorgeWGeog](https://twitter.com/GeorgeWGeog).
